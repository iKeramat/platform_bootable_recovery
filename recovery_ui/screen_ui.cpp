...
/* recovery dark:  #8E44AD
   recovery light: #9A50B9
   fastbootd dark: #E298FF
   fastboot light: #DD93FA */
  ...
  switch (e) {
    ...
    case UIElement::HEADER:
      if (fastbootd_logo_enabled_)
        gr_color(0xdd, 0x93, 0xfa, 255);
      else
        gr_color(0x9a, 0x50, 0xb9, 255);
      break;
    case UIElement::MENU:
...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    ...
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : lmodroid_logo_;
    if (logo && back_icon_) {
