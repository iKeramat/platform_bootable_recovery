...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    ...
    if (lmodroid_logo_ && back_icon_) {
      auto logo_width = gr_get_width(lmodroid_logo_.get());
      auto logo_height = gr_get_height(lmodroid_logo_.get());
      auto centered_x = ScreenWidth() / 2 - logo_width / 2;
      DrawSurface(lmodroid_logo_.get(), 0, 0, logo_width, logo_height, centered_x, y);
      y += logo_height;
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  lmodroid_logo_ = LoadBitmap("logo_image");
...
std::unique_ptr<Menu> ScreenRecoveryUI::CreateMenu(const std::vector<std::string>& text_headers,
  ...
  int menu_rows = (ScreenHeight() - margin_height_*2 - gr_get_height(lmodroid_logo_.get()))
